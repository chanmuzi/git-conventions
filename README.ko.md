...
> **Skill rename은 자동으로 추적되지 않습니다.** upstream에서 skill이 rename되면 Skills CLI는 옛 디렉터리(`~/.codex/skills/{old-name}/` 등)를 그대로 남겨둡니다. 동일한 description을 가진 skill이 두 개로 보이거나, 옛 이름으로 노출될 수 있습니다. 옛 폴더를 수동으로 제거하세요:
> ```bash
