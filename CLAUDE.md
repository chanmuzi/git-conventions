...
- **Assignee**: `gh pr create`, `gh issue create` 시 반드시 `--assignee @me` 포함. 권한 부족 등으로 `--assignee`/`--label` fallback이 발생하면, drop된 플래그가 무엇이고 왜 빠졌는지 사용자에게 명시적으로 고지한다 (silent drop 금지)
