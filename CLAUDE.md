...
- **해결책**: 항목 사이에 `---` (horizontal rule)을 삽입하여 시각적 구분을 보장한다. `code-review` SKILL.md의 Terminal Format 참조.
...
| `---` | 수평선 (전체 너비) | 항목 구분에 가장 확실한 시각적 분리 |
| `<details>`, `<summary>` | **렌더링 안 됨** — raw HTML 표시 | 터미널 출력에서 절대 사용 금지 |
| 빈 줄 (list 내) | **무시됨** | 리스트 항목 분리에는 `---` 사용 |

### 원칙

- 터미널 출력 템플릿에서는 HTML 태그를 사용하지 않는다.
- 리스트 항목 간 분리가 필요하면 `---`를 사용한다 (빈 줄만으로는 불충분).
